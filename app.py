...
for id in db.getTasksId():
    task={'name':id, 'text':db.getText(id), 'urgency':db.getUrgency(id)}
    tasks.append(task)
...
def api_update_task():
    ...
    if flag==True:
        db.updateTask(taskTmp)
        response = jsonify({'message': "Successfully updated " + str(task['name'])})
        response.status_code = 200
        return response
    else:
        response = jsonify({'message': "id not in the DB "})
        response.status_code = 404
        return response
